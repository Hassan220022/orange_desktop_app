...
Port of v1 db/hashing.py with two changes:
1. Functions that take a file path raise HashingError on OSError.
2. v1 import paths (alarm_app.db.hashing vs db.hashing try/except) removed.
"""
...
def compute_file_sha256(path: str | Path) -> str:
    ...
    try:
        ...
        raise HashingError(f"Failed to hash file at {path!r}: {exc}") from exc
...
def compute_perceptual_hash(image_path: str | Path) -> str:
    ...
    try:
        ...
        raise HashingError(f"Failed to compute perceptual hash for {image_path!r}: {exc}") from exc
    except (ImportError, ModuleNotFoundError) as exc:
        raise HashingError(f"imagehash or PIL not installed: {exc}") from exc
