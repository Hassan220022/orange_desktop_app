...
function points(payload) {
  ...
  return labels.map((label, index) => ({
    label,
    value: numberValue(values[index]),
  }));
}
...
function renderLine(items) {
  ...
  const coords = items
    ...
    .join(" ");
  return `<svg viewBox="0 0 ${width} ${height}"><polyline points="${coords}" fill="none" stroke="#2563eb" stroke-width="3"/>${coords
    .split(" ")
    .filter(Boolean)
    .map((pair) => {
...
function renderScatter(items) {
  ...
  return `<svg viewBox="0 0 ${width} ${height}">${items
    .map((item) => {
      const x = (numberValue(item.x) / maxX) * (width - 30) + 15;
      const y =
        height -
        (numberValue(item.y ?? item.value) / maxY) * (height - 30) -
        15;
...
function renderDonut(items) {
  const total =
    items.reduce(
      (sum, item) => sum + Math.max(0, numberValue(item.value)),
      0,
    ) || 1;
  let offset = 0;
  const colors = [
  ...
  // pathLength="100" lets us use 0-100 dash percentages directly instead of
  // having to compute against the actual SVG circumference (~440 for r=70
  // with stroke-width 34), which is what made pie/donut slices render as
  // striped / repeated arcs in earlier rounds.
  const rings = items
    .map((item, index) => {
      const value = Math.max(0, numberValue(item.value));
      const dash = (value / total) * 100;
      const circle = `<circle r="70" cx="110" cy="110" pathLength="100" fill="transparent" stroke="${colors[index % colors.length]}" stroke-width="34" stroke-dasharray="${dash} ${100 - dash}" stroke-dashoffset="${-offset}"/>`;
      offset += dash;
      return circle;
    })
    .join("");
...
function renderLegend(items) {
  return items
    .map(
      (item) =>
        `<div class="muted">${escapeHtml(item.label)}: ${escapeHtml(item.value)}</div>`,
    )
    .join("");
}
